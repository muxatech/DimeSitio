...
export default function SuscripcionPage() {
  ...
  useEffect(() => {
    if (isError && error instanceof Error && error.message === NO_SESSION_ERROR) {
  ...
  async function handleCheckout(restaurantId: string) {
    ...
    try {
      const url = await createCheckoutSession(restaurantId, locale)
  ...
  async function handlePortal(restaurantId: string) {
    setLoadingId(restaurantId)
    try {
      const url = await createPortalSession(restaurantId, locale)
      window.location.assign(url)
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : t('portalError'))
    } finally {
      setLoadingId(null)
    }
  }
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isActive ? (
                    ...
                    >
                      ...
                      {isPending ? t('verifying') : t('activate')}
                    </button>
                  )}
                </div>
              </motion.div>
